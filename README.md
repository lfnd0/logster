...
<p align="center">
  <img alt="Dagster" src="https://img.shields.io/badge/dagster-v.0.14.15-4f43dd">
</p>
...
| `logster/`       | A Python directory that contains code for your Dagster repository                 |
| `logster_tests/` | A Python directory that contains tests for `logster`                      |
...
pyenv virtualenv $PYTHON_VERSION logster
pyenv activate logster
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name logster python=PYTHON_VERSION
conda activate logster
```
...
Tests can be found in `logster_tests` and are run with the following command:

```bash
pytest logster_tests
```

As you create Dagster ops and graphs, add tests in `logster_tests/` to check that your
